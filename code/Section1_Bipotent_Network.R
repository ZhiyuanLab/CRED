# '=== Header Start ==='
# Title:       Section1_Bipotent_Network
# Author:      Wanqi Li
# Date:        20240318
# Purpose:     analysis and figure for Bipotent Network mentioned in Section 1
# '=== Header End ==='

# config --------
setwd("D:/wq/zhiyuanlab/20230717-CRED/CRED_project/code/")
data_folder <- "../data/Simulation_Bipotent_Network/"
image_folder <- "../data/Simulation_Bipotent_Network/image/"
...
Plot_Scatter_XY <- function(Exp_tsne_Mat, name_x, name_y, name_color, lab_color, max_color = NA,
                            max_x = NA, min_x = NA,
                            name_title,
...
regulon_r <- read.csv(paste0(data_folder, "regulon_remove_overlap.csv"),
                    stringsAsFactors = FALSE,
                    header = T, encoding = "UTF-8")
regulon_r_Viper <- Format_Regulon_DoRothEA2Viper(regulon_r)

# ----
# 1 calculate activity --------
ActMat <- Calculate_Activity(ExpMat, meta_info, regulon_Viper, minsize = 4)
dim(ActMat) # [1]    2 2000
...
# remove XY overlap regulons
ActMat_r <- Calculate_Activity(ExpMat, meta_info, regulon_r_Viper, minsize = 4)
dim(ActMat_r) # [1]    2 2000
write.csv(ActMat_r, file = paste0(data_folder, "Activity_remove_overlap.csv"),
          na = 'NA', fileEncoding="UTF-8")
ActMat_r <- read.csv(paste0(data_folder, "Activity_remove_overlap.csv"), stringsAsFactors = FALSE,
                   row.names = 1, header = T, encoding = "UTF-8", check.names = FALSE)
ActMat_r <- as.data.frame(t(ActMat_r))

# ----
# 2 plot dimentional reduction --------
# do reduction and save position
tsne_df <- Do_Dimentional_Reduction(ExpMat, 
                                    method = "t-SNE", p = 350, seed = 25,
...
para_all_time_max <- 400
Exp_tsne_Mat <- Exp_tsne_Mat[Exp_tsne_Mat$PseudoTime <= para_all_time_max,]
dim(Exp_tsne_Mat) # [1] 1010   19

para_focus_time_max <- 250
para_focus_time_min <- 200
Exp_tsne_Mat_focus <- Exp_tsne_Mat[Exp_tsne_Mat$PseudoTime <= para_focus_time_max
                                   & Exp_tsne_Mat$PseudoTime >= para_focus_time_min,]
dim(Exp_tsne_Mat_focus) # [1] 125  19

# plot reduction
# Fig. 1C
Plot_Dimentional_Reduction(Exp_tsne_Mat, name_title = "t-SNE visualization\nbased on gene expression", 
                           name_color = "PseudoTime", lab_color = "PseudoTime", max_color = 400,
                           flag_save_plot = TRUE, path_reduction_plot = paste0(image_folder, "tsne"), type = ".pdf")
...
# Supp. Fig. 1A
focus_gene <- "Y" # change this
Plot_Dimentional_Reduction(Exp_tsne_Mat, name_title = paste0("Gene expression of ",focus_gene, "\nwith t-SNE embedding"), 
...
Act_r_tsne_Mat <- merge(ActMat_r, tsne_df,
                      by = 'row.names')
rownames(Act_r_tsne_Mat) <- Act_r_tsne_Mat[,1]
colnames(Act_r_tsne_Mat)[1] <- "cell"
Act_r_tsne_Mat$PseudoTime <- as.numeric(as.data.frame(matrix(unlist(strsplit(as.character(Act_r_tsne_Mat$cell), '_')), ncol=2, byrow=TRUE))[,2])

Act_r_tsne_Mat <- Act_r_tsne_Mat[Act_r_tsne_Mat$PseudoTime <= para_all_time_max,]
dim(Act_r_tsne_Mat) # [1] 1010   19

Act_r_tsne_Mat_focus <- Act_r_tsne_Mat[Act_r_tsne_Mat$PseudoTime <= para_focus_time_max
                                   & Act_r_tsne_Mat$PseudoTime >= para_focus_time_min,]
dim(Act_r_tsne_Mat_focus) # [1] 125   6

# plot activity on reduction
# Supp. Fig. 1B
focus_gene <- "Y" # change this
Plot_Dimentional_Reduction(Act_tsne_Mat, name_title = paste0("Gene activity of ",focus_gene, "\nwith t-SNE embedding"), 
...
Act_tsne_Mat <- Act_tsne_Mat[Act_tsne_Mat$PseudoTime <= para_all_time_max,]
dim(Act_tsne_Mat) # [1] 1010   19

Act_tsne_Mat_focus <- Act_tsne_Mat[Act_tsne_Mat$PseudoTime <= para_focus_time_max
                                   & Act_tsne_Mat$PseudoTime >= para_focus_time_min,]
dim(Act_tsne_Mat_focus) # [1] 125   6

# ----
# 3 plot scatter of X and Y --------
focus_gene1 <- "X"
focus_gene2 <- "Y"

# Fig. 1D
# expression
Plot_Scatter_XY(Exp_tsne_Mat, name_x = focus_gene1, name_y = focus_gene2, name_color = "PseudoTime", lab_color = "PseudoTime", max_color = 400,
                max_x = 3, min_x = 0,
...
Plot_Scatter_XY(Exp_tsne_Mat_focus, name_x = focus_gene1, name_y = focus_gene2, name_color = "PseudoTime", lab_color = "PseudoTime", max_color = 400, 
                max_x = 3, min_x = 0,
                name_title = paste0("Expression of gene ", focus_gene1, " and ", focus_gene2),
                flag_save_plot = TRUE, path_plot = paste0(image_folder, "scatter_focus_exp_", focus_gene1, "_", focus_gene2), type = ".pdf")

# Fig. 1E
# activity
Plot_Scatter_XY(Act_tsne_Mat, name_x = focus_gene1, name_y = focus_gene2, name_color = "PseudoTime", lab_color = "PseudoTime", max_color = 400, 
                max_x = 3, min_x = -3,
...
Plot_Scatter_XY(Act_tsne_Mat_focus, name_x = focus_gene1, name_y = focus_gene2, name_color = "PseudoTime", lab_color = "PseudoTime", max_color = 400, 
                max_x = 3, min_x = -3,
                name_title = paste0("Activity of gene ", focus_gene1, " and ", focus_gene2),
                flag_save_plot = TRUE, path_plot = paste0(image_folder, "scatter_focus_act_", focus_gene1, "_", focus_gene2), type = ".pdf")

# Supp. Fig. 1C
# remove XY overlap regulons
Plot_Scatter_XY(Act_r_tsne_Mat_focus, name_x = focus_gene1, name_y = focus_gene2, name_color = "PseudoTime", lab_color = "PseudoTime", max_color = 400, 
                max_x = 3, min_x = -3,
                name_title = paste0("Activity of gene ", focus_gene1, " and ", focus_gene2),
                flag_save_plot = TRUE, path_plot = paste0(image_folder, "scatter_focus_act_remove_overlap_", focus_gene1, "_", focus_gene2), type = ".png")
